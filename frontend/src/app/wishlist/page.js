  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8 text-slate-900">❤️ My Wishlist</h1>
        ...
        {wishlist.length === 0 ? (
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {wishlist.map(item => (
                <div key={item.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition flex flex-col">
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <p className="text-sm text-slate-600 mb-2">{item.product.category?.name}</p>
                    <div className="mb-auto mb-4">
                      <p className="text-lg font-bold text-blue-600">${item.product.price}</p>
                      {item.product.averageRating && (
                        <p className="text-sm text-yellow-500">⭐ {item.product.averageRating}</p>
                      )}
                    </div>
